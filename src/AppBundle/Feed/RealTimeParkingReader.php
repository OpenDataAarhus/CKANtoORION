...
{
    // Sensor measurements
    private const FEED_PATH_PARKING = '/api/action/datastore_search?resource_id=2a82a145-0195-4081-a13c-b0e587e9b89c';

    // Geo coding
    private const PARKING_GARAGE_LOCATIONS = [
    ...
    private const PARKING_GARAGE_NAMES = [
        ...
        foreach ($parking_array_keyed as $record) {
            if (array_key_exists($record->garageCode, self::PARKING_GARAGE_LOCATIONS)) {
                ...
                $asset['name'] = [
                    ...
                    'value' => self::PARKING_GARAGE_NAMES[$record->garageCode],
                ];

                // Location
                $location = self::PARKING_GARAGE_LOCATIONS[$record->garageCode];
                $asset['location'] = [
                    'type' => 'geo:point',
